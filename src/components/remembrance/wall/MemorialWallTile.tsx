...
export default function MemorialWallTile({
  ...
  return (
    <Link
      ...
      className={`group relative flex flex-row w-full bg-white rounded-xl shadow-sm hover:shadow-md border border-slate-200 transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1877F2] hover:-translate-y-1 ${className}`}
      aria-label={`Remember ${name}`}
    >
      <div className="relative w-32 sm:w-36 shrink-0 bg-slate-100 overflow-hidden rounded-l-xl">
        <Image
          src={imageUrl}
          alt=""
          fill
      ...
      <div className="flex flex-col flex-1 min-w-0 p-2.5 sm:p-3 text-left rounded-r-xl">
        ...
        <div className="mt-auto pt-2 flex flex-col gap-2">
          <div className="flex items-center gap-1.5">
            ...
            <div>
              ...
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-none ml-1.5">
                candles lit
              </span>
            </div>
          </div>

          <span className="flex w-full items-center justify-center rounded-lg bg-[#1877F2] px-3 py-2 text-[11px] font-bold uppercase tracking-wide text-white shadow-sm transition-colors group-hover:bg-blue-600">
            View Tribute
          </span>
        </div>
      </div>
